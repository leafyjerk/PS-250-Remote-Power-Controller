// ============================================================================
// BC-250 Power Controller - LIGHTWEIGHT / STEAM CONTROLLER ONLY
// ============================================================================
// Stripped-down build: no WiFi, no web server, no OTA, no LittleFS, no
// Bluetooth / PS5 controller support. What remains:
//   - Steam Controller puck wake (INA219 current sensing)
//   - Momentary power button (short press = shutdown, long press = force,
//     press while off = power on)
//   - NeoPixel animation while the BC-250 is running
//   - PC state monitoring + relay power state machine (pc_control.h)
//
// IMPORTANT - board core: with Bluepad32 gone, compile this against the
// PLAIN esp32 board group, NOT the esp32_bluepad32 group. The Bluepad32 core
// auto-starts its Bluetooth stack before setup() even runs, so staying on
// that core keeps the radio burning power for nothing.
//
// Sketch folder: remove web_server.h, ps5_simple.h, and the data/ folder
// from the sketch directory (or at least don't include them). Keep pins.h,
// neopixel.h, pc_control.h, version.h.
// ============================================================================

#include <WiFi.h>            // only used to force the radio OFF
#include <Wire.h>
...
void setup() {
    // Drop the CPU to 80 MHz - with the radios off, this is more than enough
    // for 50 Hz I2C sampling, the relays, and the NeoPixel strip, and it
    // roughly halves the ESP32's power draw. Comment out to run stock 240.
    setCpuFrequencyMhz(80);

    Serial.begin(115200);
    delay(1000);
    Serial.println("\n\n=== BC-250 STARTING (lightweight build) ===");

    // Radios off - nothing in this build uses WiFi or Bluetooth
    WiFi.mode(WIFI_OFF);
...
void loop() {
    ...
    if (now - lastStatePrint >= 60000) {
        ...
        Serial.print(pcIsOn ? "ON" : "OFF");
        Serial.print(", puck: ");
        Serial.print(ina219Present ? (puckArmed ? "ARMED" : "disarmed") : "NO SENSOR");
        Serial.println();
        lastStatePrint = now;
    }
